...
public final class CoreProtect {
    ...
    private static final Pattern DATA_LINE_PATTERN = Pattern.compile("(\\S*)(?: ago)* (-|\\+) (.\\S*) (.*) (.*)\\.");
        ...
        try (Statement statement = Database.getConnection(false, 1000).createStatement()) {
            ...
            for (String line : lookup) {
                ...
                if ((matcher = NO_DATA_PATTERN.matcher(stripped)).matches()) {
                    ...
                    message.append("\n").append(String.join("\n", Message.INSPECT_DATA_HEADER.getMessages(matcher.group(2), matcher.group(3), matcher.group(4))));
